import { useState } from "react";
import { useGetUserProfileQuery, useUpdateProfileMutation } from "../../api/profileApi";
import { useDispatch, useSelector } from "react-redux";
import './profile.css'
import UserOrders from "./ProfileOrders";
import { LiaSignOutAltSolid } from "react-icons/lia";
import { supabase } from "../../supabasae/createclient";
import { logout } from "../../Redux/authSlice";
import { useNavigate } from "react-router";
import { uploadImage } from "../../utils/uploadeImage";

export default function ProfilePage() {
    const { user } = useSelector(state => state.auth)
    const { data: profile = [], isLoading } = useGetUserProfileQuery(user?.id, { skip: !user?.id });
    const [updateProfile] = useUpdateProfileMutation()
    const dispatch = useDispatch();
    const navigate = useNavigate()
    const userProfile = profile[0];
    const [activeTab, setActiveTab] = useState("profile");
    const [edit, setEdit] = useState(false)
    const [name, setName] = useState(null)
    const [email, setemail] = useState(null)
    const [phone, setPhone] = useState(null)


    const handelLogout = async () => {
        await supabase.auth.signOut();
        dispatch(logout())
    }


    if (isLoading) return <div className="profile-wrapper text-center">Loading...</div>;
    if (!user) navigate('/');

    const editImage = async (e) => {
        const file = e.target.files[0];
        if (!file) return;

        try {
            // رفع الصورة
            const imageProfile = await uploadImage(file);

            // تحديث البروفايل
            await updateProfile({
                id: user?.id,
                data: {
                    avatar: imageProfile
                }
            }).unwrap();
            window.location.reload()
            console.log('تم تحديث الصورة:', imageProfile);
        } catch (error) {
            console.error('خطأ أثناء رفع الصورة:', error.message);
        }
    };

    const editProfile = async () => {
        try {
            await updateProfile({
                id: user?.id,
                data: {
                    display_name: name !== null ? name : userProfile.display_name,
                    email: email !== null ? name : userProfile.email,
                    phone
                }
            }).unwrap();
            setEdit(false)
            window.location.reload()
        }
        catch (err) {
            console.log(err);

        }
    }


    return (
        <div className="profile-wrapper">
            {/* التابات */}
            <div className="tabs relative">
                <button
                    className={`tab-btn ${activeTab === "profile" ? "active" : ""}`}
                    onClick={() => setActiveTab("profile")}
                >
                    Profile
                </button>
                <button
                    className={`tab-btn ${activeTab === "orders" ? "active" : ""}`}
                    onClick={() => setActiveTab("orders")}
                >
                    Orders
                </button>
                <button>
                    <LiaSignOutAltSolid className="absolute right-3 top-3 cursor-pointer dark:text-white" size={25} onClick={handelLogout} />
                </button>
            </div>

            {/* المحتوى */}
            <div className="tab-content">
                {activeTab === "profile" && (
                    <div>
                        <div className="profile-header">
                            <img
                                src={userProfile?.avatar || "data:image/png;base64,"}
                                alt="Profile"
                                className="profile-avatar"
                                loading="lazy"
                                onClick={() => document.getElementById('fileInput').click()}
                            />
                            {
                                !edit ? (
                                    <>
                                        <h1 className="profile-name">{userProfile?.display_name || "No Name"}</h1>
                                        <p className="profile-email">{userProfile?.email}</p>
                                    </>
                                ) : (
                                    <>
                                        <input type="text" className="border-2 border-indigo-500" placeholder={userProfile.display_name} onChange={(e) => setName(e.target.value)} />
                                        <input type="text" className="border-2 border-indigo-500 mt-2" placeholder={userProfile.email} onChange={(e) => setemail(e.target.value)} />
                                    </>

                                )
                            }
                        </div>

                        <div className="profile-info">
                            <div className="info-item">
                                <span className="info-label">Role</span>
                                <span className="info-value">{userProfile?.role || "User"}</span>
                            </div>
                            <div className="info-item">
                                <span className="info-label">Created At</span>
                                <span className="info-value">{new Date(user?.created_at).toLocaleDateString()}</span>
                            </div>
                            <div className="info-item">
                                <span className="info-label">Phone</span>
                                {edit ? <input type="text" className="border-2 border-indigo-500" placeholder={'+20 000 000 000'} onChange={(e) => setPhone(e.target.value)} />
                                    :
                                    <span className="info-value">{userProfile?.phone || "Not provided"}</span>
                                }
                            </div>
                        </div>

                        {
                            edit ?
                                <button className="edit-btn" onClick={() => editProfile()}>SAVE</button>
                                :
                                <button className="edit-btn" onClick={() => setEdit(true)}>Edit Profile</button>
                        }
                    </div>
                )}

                {activeTab === "orders" && (
                    <div>
                        <h2 className="text-xl font-bold mb-4 dark:text-white">Your Orders</h2>
                        <p className="text-gray-500 dark:text-white">Here will be the user's past orders.</p>
                        <UserOrders />
                    </div>
                )}
            </div>
            <input
                type="file"
                id="fileInput"
                className="hidden"
                accept="image/*"
                onChange={editImage}
            />
        </div >
    );
}
